...
def aggregate_forest_cover(raster_file, output_file, window_size):
    ...
    with rasterio.open(raster_file) as src:
        ...
        for wx in tqdm(range(n_windows_x), desc=f"Processing file: {raster_file}"):
            for wy in range(n_windows_y):
                ...
                # Check for forest cover class
                mask = (200 <= data) & (data < 400)
                
                # Sum up the forest cover pixels
...
def main():
    ...
    aggregate_forest_cover(args.input_file, args.output_file, args.grid_size)
