...
gdal_calc.py -A "${out_dir}/merged_file.tif" -B $prev_year_gt --allBands A --outfile="${out_dir}/merged_file_fc-processed.tif" --calc="A*(B==0)" --NoDataValue=-1

# Cut the file to Amazon biome
gdalwarp -multi -dstnodata -1 -cutline $shp_file -crop_to_cutline -of GTiff "${out_dir}/merged_file_fc-processed.tif" "${out_dir}/merged_file_fc-processed_cut.tif"

# Threshold the predictions
for threshold in "${thresholds[@]}"
do
    gdal_calc.py -A "${out_dir}/merged_file_fc-processed_cut.tif" --allBands A --outfile="${out_dir}/merged_file_fc-processed_cut_${threshold}.tif" --calc="A>=${threshold}" --NoDataValue=-1

    # Convert maps to m^2
    gdal_calc.py -A "${out_dir}/merged_file_fc-processed_cut_${threshold}.tif" --allBands A --outfile="${out_dir}/merged_file_fc-processed_cut_${threshold}_m2.tif" --calc="A*900" --NoDataValue=-1
...
# Convert maps to m^2
gdal_calc.py -A "${out_dir}/merged_file_fc-processed_cut.tif" --allBands A --outfile="${out_dir}/merged_file_fc-processed_cut_m2.tif" --calc="A*900" --NoDataValue=-1
